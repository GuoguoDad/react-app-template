  ...
  const renderHeader = () => {
    return (
      <div className="header">
        ...
        <div className="middle center">拆包</div>
    ...
    return (
      <div className="main-goods-info-contanier">
        <div className="goods-info-item row">
          ...
          <div className="main-goods-name">{unpackingModalData?.mainGoodsInfo?.goodsName ?? ''}</div>
        </div>
      ...
      <>
        <div className="main-goods-info-contanier second-container">
          <div className="goods-info-item row">
            ...
            <div className="main-goods-name">{unpackingModalData?.subGoodsInfo?.goodsName ?? ''}</div>
          </div>
          <div className="goods-info-item row align-items-center just-content-space-between">
            <span className="inventory-num">现有库存: {unpackingModalData?.subGoodsInfo?.inventoryNum ?? 0}</span>
            <span className="generate-txt">
              生成数: <span className="generate-num">{subNum}</span>
            </span>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="row center">
        <img className="arrow_donw" src={ArrowDown} />
      </div>
      {renderSubGoodsInfo()}
      <Button onClick={() => submit()} className="sure-btn" type="primary">
        确定
      </Button>
    </Modal>
  )
